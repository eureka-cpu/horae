...
A person continues to add time the quick way — typing a duration in the Week grid, or using the "+" form without touching a time — and everything keeps working. Those entries have no time of day; on the calendar they gather in a clearly separated area for the day rather than pretending to sit at a specific hour, and their day/week totals are unchanged.
...
**Independent Test**: Add an entry via the Week grid (duration only), open Calendar view, and confirm the entry appears in the day's untimed area and all totals are unchanged; confirm existing entries created before this feature still display and edit normally.

**Acceptance Scenarios**:

1. **Given** an entry with a duration but no start time, **When** the user views Calendar view, **Then** the entry appears in the day's untimed area, not at an arbitrary hour, and counts toward the day and week totals.
2. **Given** a timed entry, **When** the user clears its start time in the entry form, **Then** it becomes an untimed entry and moves to the untimed area with its duration unchanged.
...
- **FR-006**: The Calendar view MUST present entries that have no start time in a distinct untimed area for that day, without assigning them a misleading hour, and MUST keep them included in all totals.
...
- **Optional per entry, no account-wide mode**: a start time is optional on each entry; there is no separate account setting that forces "start/end time" vs "duration" tracking. This keeps the feature additive and simpler than a global mode toggle.
...
- **Untimed area placement**: entries without a start time are shown together in a per-day untimed area (e.g. pinned above the timed grid). The precise visual treatment is a design detail for planning.

...
1. **Re-validate Spec Quality Checklist** (if it exists):
   ...
   - If it exists:
     ...
     1. Identify all GitHub task-list checkbox lines — lines matching `- [ ]`, `- [x]`, or `- [X]` (case-insensitive, tolerant of leading whitespace for nested items) outside of code fences. Ignore all other content (headings, notes, non-checkbox bullets, metadata).
     1. For each checkbox line, record its current marker state (checked or unchecked) and item text into a before-snapshot list.
     1. Re-evaluate each checkbox item against the **updated** spec (the version just saved in step 7).
     1. For each checkbox item, update only if the checked/unchecked state actually changes:
     ...
     1. Compare the before-snapshot with the current state to compute three lists for the Completion Report:
